...
import com.algorithm.RandomForest;
import com.algorithm.RandomForestClassifier;
...
public class GiveReviewController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String review=request.getParameter("loc_review");
		list.add(review);
		
		RandomForestClassifier<String, String> sentiment = new RandomForest<String, String>();
		final String[] positiveText = {"love", "like", "happy", "good", "sunny", "able", "average" , "newsletter", 
				...
				//.split("\\s");
		sentiment.learn("Positive", Arrays.asList(positiveText));
	    ...
	    sentiment.learn("Negative", Arrays.asList(negativeText));
	    String words="";		
	    ...
	    if(list.size()>0){
			...
			for(int i=0;i<list.size();i++){	
				...
				String bayesResult = sentiment.classify(Arrays.asList(word)).getCategory();
